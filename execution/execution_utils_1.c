...
int builtin_cmd(char *command, char **args, t_env *head)
{
	...
	if (ft_strcmp(command, "export") == 0)
	{
		if (args == NULL)
		{
    		t_env *tmp;
			tmp = copy_env(head);
			return (sort_env(&tmp), print_env(tmp, 1), free_env(tmp), 1);
    	}
		else
    		return (set_env(args, head), print_env(head, 1), 1);
    	return 1;
	}
	return 0;
}
...
void delete_env(t_env *head, char *name)
{
	...
	while (tmp)
	{
		...
		{
      		if (prev == NULL)
        		return (head = tmp->next, free(tmp));
			else
        		return (prev->next = tmp->next, free(tmp));
	    }
		prev = tmp;
		tmp = tmp->next;
	}
